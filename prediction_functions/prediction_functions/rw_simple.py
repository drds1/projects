...
def rw(ti,yi,si=0,tgi = -1,fbreak=-1,custom_freqs=[],nits = 1):
 ...
 if nits > 1:
  ...
  ygrid = (np.dot(swhires,parm[1::2]) + np.dot(cwhires,parm[0::2])) * ystd + ymean
  sd = np.sqrt(var)
 ...
 return(tgrid,ygridop,f,ckout*ystd,skout*ystd,rcoef)
