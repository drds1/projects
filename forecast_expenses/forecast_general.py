...
#
#
#trend    = 0.0 + 0.0*(t-tref) + 1.e-5*(t-tref)**2 + 0.0*(t-tref)**3 
##grad*(x-t[-1]) + y1
#nt = np.shape(t)[0]
#noise = np.random.randn(nt)*noiseamp
#signal = x + trend + noise
#
#
...
#orderin = (1,1,0)
#model   = sm.tsa.statespace.SARIMAX(endog=signal,order=orderin,seasonal_order=(0,1,0,120),trend='c',enforce_invertibility=False)
#results = model.fit()
