...
int colfill[M][N],colsave[M][N],bnow[N2];


for (int i1=0;<M;i1++){
for (int i2 =0;<N;i2++) colfill[i1][i2] = 0;
}
// CXARYY on FROM HERE TOMOEERROW
...
// outer loop over bricks in ascending order from high value down
for (ib=0;ib<B;ib=ib+1){
ibnow = ibsort[B-1-ib];
cout << "trying brick..."<<ibnow<<endl;
